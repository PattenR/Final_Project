...
#from differential_privacy.dp_sgd.dp_optimizer
#import dp_optimizer
#from differential_privacy.dp_sgd.dp_optimizer
#import dp_pca
#from differential_privacy.dp_sgd.dp_optimizer
...
#from differential_privacy.dp_sgd.dp_optimizer
#import utils
#from differential_privacy.privacy_accountant.tf
#import accountant
...
tf.flags.DEFINE_integer("num_training_steps", 200,
                        "The number of training steps."
                        "This counts number of lots.")
...
#                      "used if accountant_type is Amortized.")
tf.flags.DEFINE_float("eps", 500,
                      "Start privacy spending for one epoch of training, "
                      "used if accountant_type is Amortized.")
tf.flags.DEFINE_float("end_eps", 500,
                      "End privacy spending for one epoch of training, "
                      "used if accountant_type is Amortized.")
...
#tf.flags.DEFINE_float("delta", 1e-1,
#                      "Privacy spending for training. Constant through "
#                      "training, used if accountant_type is Amortized.")
tf.flags.DEFINE_float("delta", 10,
                      "Privacy spending for training. Constant through "
                      "training, used if accountant_type is Amortized.")
tf.flags.DEFINE_float("sigma", 40.0,
                      "Noise sigma, used only if accountant_type is Moments")
#tf.flags.DEFINE_float("delta", 0,
#                      "Privacy spending for training. Constant through "
#                      "training, used if accountant_type is Amortized.")
#tf.flags.DEFINE_float("sigma", 0,
#                      "Noise sigma, used only if accountant_type is Moments")

# Flags that control privacy spending for the pca projection
# (only used if --projection_dimensions > 0).
#tf.flags.DEFINE_float("pca_eps", 0.5,n
#                      "Privacy spending for PCA, used if accountant_type is "
#                      "Amortized.")
tf.flags.DEFINE_float("pca_eps", 0.99,
                      "Privacy spending for PCA, used if accountant_type is "
                      "Amortized.")
#tf.flags.DEFINE_float("pca_delta", 0.005,
#                      "Privacy spending for PCA, used if accountant_type is "
#                      "Amortized.")
tf.flags.DEFINE_float("pca_delta", 0.9,
                      "Privacy spending for PCA, used if accountant_type is "
                      "Amortized.")
tf.flags.DEFINE_float("pca_sigma", 70.0,
                      "Noise sigma for PCA, used if accountant_type is Moments")

tf.flags.DEFINE_string("target_eps", "0.125,0.25,0.5,1,2,4,800",
...
tf.flags.DEFINE_float("target_delta", 10,
                      "Maximum delta for --terminate_based_on_privacy.")
...
def MnistInput(mnist_data_file, batch_size, randomize, mal_data_mnist=False):
...
#  if(mal_data_mnist):
#      image, label, num_targets = mal_data_synthesis(image)
#      print("num_targets, %d\n", num_targets)
  if randomize:
...
def Eval(mnist_data_file, network_parameters, num_testing_images,
         randomize, load_path, save_mistakes=False, mal_data=False):
  ...
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    images, labels = MnistInput(mnist_data_file, batch_size, randomize, mal_data_mnist=mal_data)
  ...
  with tf.Graph().as_default(), tf.Session() as sess, tf.device('/cpu:0'):
    ...
    if network_parameters.projection_type == "PCA":
      with tf.variable_scope("pca"):
        ...
        all_data_mal, _ = MnistInput(mnist_train_file, NUM_TRAINING_IMAGES, False, mal_data_mnist=True)
        all_data = tf.concat([all_data, all_data_mal], 0)
    ...
    for step in xrange(num_steps):
      ...
      if (eval_steps > 0 and (step + 1) % eval_steps == 0) or should_terminate:
        ...
        train_accuracy_mal, _ = Eval(mnist_train_file, network_parameters,
                         num_testing_images=NUM_TESTING_IMAGES,
                         randomize=False, load_path=save_path, mal_data=True)
        sys.stderr.write("train_accuracy: %.2f\n" % train_accuracy)
        sys.stderr.write("train_accuracy_mal: %.2f\n" % train_accuracy_mal)
        ...
        test_accuracy_mal, mistakes = Eval(mnist_train_file, network_parameters,
                               num_testing_images=NUM_TESTING_IMAGES,
                               randomize=False, load_path=save_path,
                               save_mistakes=FLAGS.save_mistakes, mal_data=True)
        sys.stderr.write("eval_accuracy: %.2f\n" % test_accuracy)
        sys.stderr.write("eval_accuracy_mal: %.2f\n" % test_accuracy_mal)
